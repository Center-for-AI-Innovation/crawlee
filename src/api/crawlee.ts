        ...
        crawler = new PlaywrightCrawler({
          ...
          async requestHandler({ request, response, page, enqueueLinks, log }) {
            ...
            if (request.loadedUrl) {
              ...
              // the corpus. The bridge POST is fire-and-forget, so we log every skip with a
...
// articles are long and may legitimately mention these phrases (the user opted into
// title+content detection knowing this tradeoff, so we bound the false-positive risk
// by length rather than dropping content matching entirely).
...
// Decide whether a crawled page should be SKIPPED (not POSTed to the bridge / not ingested).
...
function shouldSkipIngest(title: string, content: string, status?: number): string | null {
  ...
  if (text.length <= SHORT_CONTENT_CHARS && ERROR_CONTENT_PATTERNS.some((re) => re.test(text))) {
